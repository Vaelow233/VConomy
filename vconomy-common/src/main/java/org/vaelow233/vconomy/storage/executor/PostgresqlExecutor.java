...
public class PostgresqlExecutor extends DatabaseExecutor {

    private static final String ADD_PLAYER_BALANCE = "INSERT INTO \"vconomy_balance\" (uuid, name, \"%1$s\") VALUES (?, ?, ?)\n" +
            "ON CONFLICT (uuid) DO UPDATE SET \"%1$s\" = \"vconomy_balance\".\"%1$s\" + ?";

    private static final String SUBTRACT_PLAYER_BALANCE = "INSERT INTO \"vconomy_balance\" (uuid, name, \"%1$s\") VALUES (?, ?, ?)\n" +
            "ON CONFLICT (uuid) DO UPDATE SET \"%1$s\" = \"vconomy_balance\".\"%1$s\" - ?";

    private static final String SET_PLAYER_BALANCE = "INSERT INTO \"vconomy_balance\" (uuid, name, \"%1$s\") VALUES (?, ?, ?)\n" +
            "ON CONFLICT (uuid) DO UPDATE SET \"%1$s\" = EXCLUDED.\"%1$s\"";

    private static final String RESET_PLAYER_BALANCE = "INSERT INTO \"vconomy_balance\" (uuid, name, \"%1$s\") VALUES (?, ?, 0)\n" +
            "ON CONFLICT (uuid) DO UPDATE SET \"%1$s\" = 0";
    ...
    private static final String CREATE_RECORD_TABLE = "CREATE TABLE IF NOT EXISTS \"vconomy_record\" (\n"
            ...
            + "  \"balance\" NUMERIC(28, 4) NOT NULL\n"
            + ")";

    private static final String BALANCE_ADD_COLUMN = "ALTER TABLE \"vconomy_balance\" ADD COLUMN \"%1$s\" NUMERIC(%2$d, %3$d) " +
